  ...
  'inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0',
  {
    variants: {
      variant: {
        ...
        destructive: 'border border-black text-black hover:bg-gray-100 dark:border-white dark:text-white dark:hover:bg-gray-800',
        outline: 'border border-gray-400 bg-white hover:bg-gray-100 hover:text-black dark:bg-transparent dark:border-gray-600 dark:text-white dark:hover:bg-gray-800 dark:hover:text-white',
        secondary: 'bg-gray-200 text-black hover:bg-gray-300 dark:bg-gray-800 dark:text-white dark:hover:bg-gray-700',
        ghost: 'text-black hover:bg-gray-100 hover:text-black dark:text-white dark:hover:bg-gray-800 dark:hover:text-white',
...
>(({ className, ...props }, ref) => {
  return (
    <textarea
      className={cn(
        'flex min-h-[80px] w-full rounded-md border border-gray-400 bg-white px-3 py-2 text-base ring-offset-white placeholder:text-gray-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gray-600 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 md:text-sm text-black dark:bg-gray-900 dark:border-gray-700 dark:text-white dark:placeholder:text-gray-400 dark:ring-offset-gray-900 dark:focus-visible:ring-gray-300',
        className,
      )}
      ref={ref}
      {...props}
  ...
  const suggestedActions = [
    {
      ...
      action: 'Can you explain the code for this project step-by-step?',
    },
    {
      title: 'Circuit help',
      label: 'troubleshoot connections',
      action: 'My circuit is not working. How can I troubleshoot the connections?',
    },
    {
      title: 'Code logic',
      label: 'how delay() works',
      action: 'What happens if I change the delay() values in the code?',
    },
    {
      title: 'Upgrade ideas',
      label: 'make it advanced',
      action: 'How can I upgrade this project to make it more advanced?',
    },
    {
      title: 'Component info',
      label: 'what is a sensor?',
      action: 'Can you explain how the sensors in this project work?',
    },
    {
      title: 'Real-world use',
      label: 'where is this used?',
      action: 'What are some real-world applications of this project?',
    },
  ];
        ...
        >
          <Button
            ...
            className="text-left border rounded-2xl px-4 py-3.5 text-sm flex-1 gap-1 sm:flex-col w-full h-auto justify-start items-start
                       border-slate-200 bg-white hover:bg-slate-50 text-slate-900 hover:text-slate-950 dark:bg-slate-900 dark:border-slate-800 dark:text-slate-300 dark:hover:bg-slate-800 dark:hover:text-white transition-all duration-200 shadow-sm hover:shadow-md"
          >
...
const PreviewAttachment = ({
  ...
  return (
    <div data-testid="input-attachment-preview" className="flex flex-col gap-1">
      <div className="w-20 h-16 aspect-video bg-gray-200 dark:bg-gray-800 rounded-md relative flex flex-col items-center justify-center overflow-hidden border border-gray-300 dark:border-gray-700">
        {contentType?.startsWith('image/') && url ? (
          ...
          <div className="flex items-center justify-center text-xs text-gray-600 dark:text-gray-400 text-center p-1">
        ...
        {isUploading && (
          <div
            ...
            className="animate-spin absolute text-gray-500 dark:text-gray-400"
          >
...
function PureAttachmentsButton({
  ...
  return (
    <Button
      ...
      className="rounded-md rounded-bl-lg p-[7px] h-fit border border-gray-300 dark:border-gray-700 hover:bg-gray-200 dark:hover:bg-gray-800"
      onClick={(event) => {
...
function PureStopButton({ onStop }: { onStop: () => void }) {
  return (
    <Button
      ...
      className="rounded-full p-1.5 h-fit border border-black text-white dark:border-white dark:text-black"
      onClick={(event) => {
        event.preventDefault();
        onStop();
      }}
  ...
  return (
    <div className={cn("relative w-full flex flex-col gap-4", className)}>
      ...
      <Textarea
        ...
        ref={textareaRef}
        placeholder="Ask Heyro anything..."
        value={input}
        onChange={handleInput}
        className={cn(
          'min-h-[24px] max-h-[calc(75dvh)] overflow-y-auto resize-none rounded-3xl !text-base pt-4 pb-12 px-6',
          'bg-white border border-slate-200 text-slate-900 placeholder:text-slate-400 dark:bg-slate-900 dark:border-slate-800 dark:text-slate-100 dark:placeholder:text-slate-500 focus-visible:ring-slate-400', 
          className,
        )}
        rows={1}
        autoFocus
